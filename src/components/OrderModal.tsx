  ...
  return (
    ...
    >
      <div className="bg-white rounded-t-3xl sm:rounded-2xl w-full sm:max-w-md p-6 shadow-2xl relative max-h-[95vh] overflow-y-auto">
        ...
        {/* Upsell — показываем только если это не бандл */}
        {product.id !== 'bundle-all' && (
          <button
            type="button"
            onClick={() => setProduct(bundleProduct)}
            className="w-full mb-4 bg-amber-50 border border-amber-300 rounded-xl px-4 py-3 text-left hover:bg-amber-100 transition-colors"
          >
